...
FLOAT FE_offset_z=0;
FLOAT FE_scale=1;
FLOAT FE_scale_z=0.5;
FLOAT FE_base =0;
FLOAT FE_kOfcenter=20;
FLOAT FE_fov=20;
D3DXVECTOR3 FE_offset;
FindEye FE_test;
DWORD FE_tests=2;
HANDLE FE_one;
void thread(void* para)
{
	FindEye* FE_one=(FindEye*)para;
	FE_one->Run();
}
...
//  Global variables
//--------------------------------------------------------------------------------------
CModelViewerCamera                  g_Camera1;                   //  A model viewing camera
...
// UI control IDs
//--------------------------------------------------------------------------------------
#define IDC_FE_scale                                         13
#define IDC_FE_scale_STATIC                                         14
#define IDC_FE_scale_Z                                        15
#define IDC_FE_scale_Z_STATIC                                        16
#define IDC_FE_kOfcenter                                        17
#define IDC_FE_kOfcenter_STATIC                                        18
#define IDC_FE_FOV                                        19
#define IDC_FE_FOV_STATIC                                        20
...
{
	//
    //  Update the camera's position FE_base d on user input 
    //
    g_Camera.FrameMove( fElapsedTime );
	/*D3DXVECTOR3 eyePt=*g_Camera.GetEyePt();
	D3DXVECTOR3 lookAtPt=*g_Camera.GetLookAtPt;
	D3DXVECTOR3 upVector=*g_Camera.;*/
	FLOAT aspect=1.33333;
	FLOAT eye1x=FE_test.outs[0][0];
	FLOAT eye1y=FE_test.outs[0][1];
	FLOAT eye2x=FE_test.outs[1][0];
	FLOAT eye2y=FE_test.outs[1][1];
	FE_offset_x=-((eye1x+eye2x)/2-0.5)*FE_scale;
	FE_offset_y=-((eye1y+eye2y)/2-0.5)*aspect*FE_scale;
	FLOAT distance=sqrt((eye2x-eye1x)*(eye2x-eye1x)*aspect*aspect+(eye2y-eye1y)*(eye2y-eye1y))*FE_scale*FE_scale_z;
	if(FE_base==0)
		FE_base=distance;
	FE_offset_z=distance-FE_base;
	D3DXVECTOR3		newoffset(FE_offset_x,FE_offset_y,FE_offset_z);
	D3DXVECTOR3		delta=newoffset-FE_offset;
	FE_offset=newoffset;
	...
	D3DXVec3Normalize(&right,g_Camera.GetWorldRight());
	D3DXVec3Normalize(&up,g_Camera.GetWorldUp());
	D3DXVec3Normalize(&ahead,g_Camera.GetWorldAhead());
	
	eyePt=*g_Camera.GetEyePt();
	center=FE_kOfcenter*ahead+eyePt;
	delta*=5;
	eyePt+=delta.x*right+delta.y*up+delta.z*ahead;
	lookAtPt=center+center-eyePt;
	g_Camera.SetViewParams(&eyePt,&lookAtPt);
	g_Camera.SetProjParams(FE_fov*3.1416/180,aspect,1,10000);
...
SetupMSAAOptions()
{
    ...
    uMaxQuality = pDeviceSettingsCombo->multiSampleQualityList.GetAt( 1 );
    ...
    for( UINT uCount = 0; uCount < (UINT)pDeviceSettingsCombo->multiSampleCountList.GetSize(); ++uCount ) {
        ...
        WCHAR str[50];
        StringCchPrintf( str, 50, L"%u", uMSAACount );
    ...
    pMultisampleCountCombo->SetSelectedByIndex( 0 );
    ...
    for( UINT uQuality = 0; uQuality < uMaxQuality; ++uQuality ) {
        WCHAR strQuality[50];
        StringCchPrintf( strQuality, 50, L"%d", uQuality );
    ...
    pMultisampleQualityCombo->SetSelectedByData( ULongToPtr( 1 ) );
    ...
    void*                       pUserContext )
{
    ...
    //
    D3DXVECTOR3 vecEye( 0.0f, 4.8f, -15.3f );
    ...
    //
    //  Re-populate the MSAA ui FE_base d on the current device
    //
    hr = SetupMSAAOptions();
    V_RETURN( hr );
    ...
    void*                       pUserContext )
{
   ...
   /* g_Camera.SetWindow( 
    ...
    //
    //  Reset the ui positions FE_base d on the current swapchain size.
    //
    g_HUD.SetLocation( 
    ...
    void*           pUserContext )
{
    ...
    //
    //  Clear and set the render targets FE_base d on the user selections
    //
    if( 1 == g_MSAASettings.Count ) 
	{
        pd3dDevice->ClearRenderTargetView( 
	...
	D3DXMatrixIdentity(&mOffset);
		mOffset(3,0)=FE_offset_x;
		mOffset(3,1)=FE_offset_y;
		mOffset(3,2)=FE_offset_z;

    mWorldViewProj =mView*mOffset*mProj;
    ...
    //
    //  Create the mips so we can sample the levels FE_base d on distance from
...
//==========================================================
		case IDC_FE_scale: {
            ...
            FE_scale = (FLOAT)pSlider->GetValue() / 200.f;
			swprintf_s( wszOutput, 1024, L"FE_scale :  %6.3f",FE_scale );		
			g_SampleUI.GetStatic(IDC_FE_scale_STATIC)->SetText(wszOutput);
		...
		case IDC_FE_scale_Z: {
            ...
            FE_scale_z= (FLOAT)pSlider->GetValue() / 200.f;
			swprintf_s( wszOutput, 1024, L"FE_scale_z :  %6.3f",FE_scale_z );		
			g_SampleUI.GetStatic(IDC_FE_scale_Z_STATIC)->SetText(wszOutput);
            
            break;
        }
						  case IDC_FE_kOfcenter: {

            CDXUTSlider* pSlider = (CDXUTSlider*)pControl;

            FE_kOfcenter= (FLOAT)pSlider->GetValue()/10.f;
			swprintf_s( wszOutput, 1024, L"k Of center :  %6.3f",FE_kOfcenter );		
			g_SampleUI.GetStatic(IDC_FE_kOfcenter_STATIC)->SetText(wszOutput);
            ...
            FE_fov= (FLOAT)pSlider->GetValue();
			swprintf_s( wszOutput, 1024, L"fov :  %6.3f",FE_fov );		
			g_SampleUI.GetStatic(IDC_FE_FOV_STATIC)->SetText(wszOutput);
...
InitApp()
{
    ...
    g_SampleUI.AddSlider( 
        ...
        22, 
        0, 
        (INT)g_fFocalPlaneVSDepth * 2000, 
        (INT)g_fFocalPlaneVSDepth * 1000 );

//====================================================
	g_SampleUI.AddStatic(IDC_FE_scale_STATIC,L"scale",  35,iY+=24,125,22);
	g_SampleUI.AddSlider( 
        IDC_FE_scale, 
        35, 
        iY += 24, 
        ...
        3000, 
        100 );
	g_SampleUI.AddStatic(IDC_FE_scale_Z_STATIC,L"Z",  35,iY+=24,125,22);
	   g_SampleUI.AddSlider( 
        IDC_FE_scale_Z, 
        35, 
        iY += 24, 
        ...
        3000, 
        100 );
	   g_SampleUI.AddStatic(IDC_FE_kOfcenter_STATIC,L"k Of center",  35,iY+=24,125,22);
	   	 g_SampleUI.AddSlider( 
        IDC_FE_kOfcenter, 
        35, 
        iY += 24, 
        ...
        1000, 
        20 );
		  g_SampleUI.AddStatic(IDC_FE_FOV_STATIC,L"fov",  35,iY+=24,125,22);
	   	 g_SampleUI.AddSlider( 
        IDC_FE_FOV, 
        35, 
        iY += 24, 
    ...
    int         nCmdShow )
{
	...
	FE_one=CreateThread(NULL,0,(LPTHREAD_START_ROUTINE)thread,&FE_test,0,&FE_tests);
